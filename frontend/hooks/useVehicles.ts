...
export function useVehicles() {
  ...
  async function load() {
    try {
      setLoading(true)
      const result = await pb.collection('vehicles').getFullList({ sort: '-created' })
  ...
  async function updateVehicle(id: string, data: Partial<Vehicle>) {
    const record = await pb.collection('vehicles').update(id, data)
    await load()
    return record
  }
...
export function useVehicle(id: string) {
  ...
  async function load() {
    ...
    setLoading(true)
    try {
      const r = await pb.collection('vehicles').getOne(id)
