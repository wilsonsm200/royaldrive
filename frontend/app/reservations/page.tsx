...
export default function ReservationsPage() {
  ...
  async function updateStatus(id: string, status: string) {
    try {
      await pb.collection('reservations').update(id, { status })
      setStatusPopup(null)
      load()
    } catch (e) {
  ...
  async function updatePayment(id: string, payment_status: string) {
    try {
      await pb.collection('reservations').update(id, { payment_status })
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0, paddingRight: '12px' }}>
                    ...
                    <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', margin: '3px 0 0' }}>
                      {r.service_type || '-'} → <strong style={{ color: '#fff' }}>{r.destination || '-'}</strong>
                    </p>
                  </div>
