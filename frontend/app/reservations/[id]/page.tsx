...
export default function ReservationDetailPage() {
  ...
  async function load() {
    try {
      const [res, custs, vehs, drvs] = await Promise.all([
        pb.collection('Reservations').getOne(id as string, { expand: 'customer,vehicle,driver' }),
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      await pb.collection('Reservations').update(id as string, {
  ...
  const vehicleName = (() => {
    if (reservation.expand?.vehicle) {
      const v = reservation.expand.vehicle
      return `${v.make} ${v.model} (${v.plate})`
    }
    const v = vehicles.find(v => v.id === reservation.vehicle)
      ...
      {!editing && (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '20px', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '18px' }}>
            {[
              { label: 'Customer', value: customers.find(c => c.id === reservation.customer)?.name || reservation.expand?.customer?.name || '-' },
              ...
              { label: 'Driver', value: reservation.expand?.driver?.name || 'Not assigned' },
        ...
        <form onSubmit={handleSave} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            ...
            <select name='customer' value={form.customer} onChange={handleChange} required style={inputStyle}>
              ...
              {customers.map(c => <option key={c.id} value={c.id}>{c.name || c.Name} — {c.phone}</option>)}
            </select>
          </div>
