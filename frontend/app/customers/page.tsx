...
export default function CustomersPage() {
  ...
  async function saveResEdit() {
    ...
    await pb.collection('reservations').update(editingResId, {
            ...
            <div key={c.id} style={{
              ...
              <div style={{ padding: '16px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', margin: 0 }}>{c.phone || '-'} {c.email ? `· ${c.email}` : ''}</p>
                ...
                {[
                  ...
                  <div key={s.label} style={{
                    ...
                    <p style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)', margin: '2px 0 0', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      {s.label === 'Bookings' ? `${s.label} ${isExpanded ? '▲' : '▼'}` : s.label}
                    </p>
                  </div>
                ))}
              </div>
                      ...
                      <div key={r.id} style={{ padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                        {editingResId === r.id ? (
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div>
                              <p style={{ fontSize: '12px', fontWeight: 600, color: '#fff', margin: 0 }}>{r.service_type || '-'} → {r.destination || '-'}</p>
                              <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)', margin: '2px 0 0' }}>{formatKES(r.amount || 0)} · {timeAgo(r.created)}</p>
                            </div>
